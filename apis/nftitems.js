...
const randomIPFS = () => {
  const IPFSUris = [
    'https://tomato-tough-squid-756.mypinata.cloud/ipfs/',
...
router.post('/fetchTokens', async (req, res) => {
  ...
  //mconsole.log('request_ip',request_ip);
  /*if (count > 80 || count === 0)
  {
    return res.json({
      status: 'failed',
...
router.post('/getSingleItemDetails', async (req, res) => {
  try {
    console.log('Request received for getSingleItemDetails:', req.body);
    ...
    console.error('Error in getSingleItemDetails:', error);
    Logger.error(error);
    return res.json({
      status: 'failed'
    });
